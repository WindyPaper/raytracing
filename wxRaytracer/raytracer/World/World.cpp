		...
		threadPool(NULL)
{
	...
	//Sphere*	sphere_ptr2 = new Sphere(Point3D(20, 0, 0), 30);
	////sphere_ptr1->set_material(matte_ptr1);
	...
	for (int r = 0; r < vres; r++)			// up
		for (int c = 0; c <= hres; c++) {	// across 	
			for (int i = 0; i < vp.num_samples; ++i)
			{
				...
				ray.o = Point3D(s * (c - hres / 2.0 + sample_point.x), s * (r - vres / 2.0 + sample_point.y), zw);
...
ShadeRec World::hit_bare_boines_objs(const Ray &ray)
{
	ShadeRec sr(*this);
	double t;
