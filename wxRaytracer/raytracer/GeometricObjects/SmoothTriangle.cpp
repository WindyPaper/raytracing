...
SmoothTriangle::get_bounding_box(void) {
	...
	return(BBox(std::fmin(std::fmin(v0.x, v1.x), v2.x) - delta, max(max(v0.x, v1.x), v2.x) + delta, 
				std::fmin(std::fmin(v0.y, v1.y), v2.y) - delta, max(max(v0.y, v1.y), v2.y) + delta,
				std::fmin(std::fmin(v0.z, v1.z), v2.z) - delta, max(max(v0.z, v1.z), v2.z) + delta));
}
