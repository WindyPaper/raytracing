		...
		shadows(m.shadows)
{
	...
	if(m.fresnel_brdf)
		fresnel_brdf = (FresnelReflector*)m.fresnel_brdf->clone();
	else  fresnel_brdf = NULL;

	if(m.fresnel_btdf)
		fresnel_btdf = (FresnelTransmitter*)m.fresnel_btdf->clone();
...
Dielectric::operator= (const Dielectric& rhs) {
	...
	if (rhs.fresnel_btdf)
		this->fresnel_btdf = (FresnelTransmitter*)rhs.fresnel_btdf->clone();

	if (rhs.fresnel_brdf)
		this->fresnel_brdf = (FresnelReflector*)rhs.fresnel_brdf->clone();
